...
class IncompressibleNavierStokesLinearOperator(LinearOperator):
    ...
    from the divergence :math:`D` and gradient :math:`G`. The resolvent is built
    around a *generator* matrix :math:`T`: in the steady case :math:`T = A`,
    while in the harmonic-balanced case
    :math:`T = A - i\,\Omega\,M` is the harmonic-resolvent generator (see
    :func:`~resolvent4py.utils.matrix.assemble_harmonic_resolvent_generator`),
    where :math:`\Omega` is the block-diagonal of perturbation frequencies
    encoding the time derivative :math:`d/dt`. For a complex shift :math:`s`,
    this class exposes two velocity-space operators:

    .. math::

        \texttt{apply}:\; P\, L^* (sM - T)\, L\, P, \qquad
        \texttt{solve}:\; L^* (sM - T)^{-1} L.
    ...
    *generator* and :meth:`solve` evaluates the *resolvent* itself; both map the
    velocity subspace to itself. Use :meth:`update_operator` to retarget a
    different shift :math:`s` without re-reading the matrices.
    ...
    non-negative Fourier coefficients of the underlying time-periodic operators
    and expanded into block-Toeplitz matrices with
    :code:`nblocks = 2 * (len(freqs) - 1) + 1` blocks, and the generator
    :math:`T = A - i\,\Omega\,M` is assembled from the perturbation frequencies
    :code:`freqs`.
    ...
    :param fname_Dm: COO file-name triplet(s) for the divergence :math:`D`,
        used together with :code:`fname_Gm` to build the Leray projector
        :math:`P`
    :type fname_Dm: Union[Tuple[str, str, str], List[Tuple[str, str, str]]]
    :param fname_Gm: COO file-name triplet(s) for the gradient :math:`G`
    :type fname_Gm: Union[Tuple[str, str, str], List[Tuple[str, str, str]]]
    :param solver_type: solver used for :math:`(sM - T)`, either
    ...
    :param freqs: non-negative perturbation frequencies for the harmonic-balanced
    ...
    def __init__(
        ...
        if freqs is None and solver_type == 'GMRES_BJACOBI':
            raise ValueError(
                ...
                "block-Jacobi preconditioner needs the harmonic block "
                "structure. Use 'MUMPS_DIRECT' for the steady operator."
            )
        ...
        if freqs is None:
            ...
            # Steady case: the generator T coincides with A (there is no
            # harmonic frequency term), so Tm aliases Am.
            self.Tm = self.Am
            ...
            if len(freqs) < len(fname_A):
                raise ValueError(
                    ...
                    f"of Fourier coefficients provided for A ({len(fname_A)})."
                )
            ...
            # assemble_harmonic_resolvent_generator expects.
            perts_freqs = np.concatenate((-np.flip(freqs[1:]), freqs))
            self.Tm = assemble_harmonic_resolvent_generator(
                self.Am, perts_freqs, self.Mm
            )
        ...
        # the resolvent operators are not needed during super().__init__. Build
        # them now so the operator is ready to apply()/solve() after
        # construction.
    ...
    def _refresh_flags(
        ...
        from the current shift :code:`self.s`. Called by :meth:`update_operator`,
        since both flags depend on :code:`s` (see :meth:`check_if_real_valued`
        and :meth:`check_if_complex_conjugate_structure`).
        """
    ...
    def update_operator(
        ...
        r"""
        Retarget the operator to the complex shift :math:`s`: rebuild both the
        resolvent (used by :meth:`solve`) and the resolvent generator (used by
        :meth:`apply`), then refresh the cached real-valued and
        complex-conjugate-structure flags, which depend on :math:`s`. Call this
        to sweep over shifts without re-reading the matrices.

        :param s: complex shift
        :type s: numpy.complex128
        """
        self._update_resolvent_operator(s)
        self._update_resolvent_generator(s)
        self.s = s
        self._refresh_flags()
    ...
    def _update_resolvent_operator(
        ...
        .. math::
            ...
            \texttt{solve}(x) = L^* (sM - T)^{-1} L\, x,

        factorizing :math:`sM - T` (:math:`T` is the generator matrix; see the
        class docstring) with the configured :code:`solver_type` and
    ...
    def _update_resolvent_generator(
        ...
        .. math::
            ...
            \texttt{apply}(x) = P\, L^* (sM - T)\, L\, P\, x,

        the divergence-free-projected forward operator. No factorization is
        needed (:math:`sM - T` is only applied), so the Leray projector
        ...
        # _update_resolvent_operator. self.Rinv wraps the previous sMg.
        ...
        self.sMg.scale(s)
        self.sMg.axpy(-1.0, self.Tm)
        # The generator applies sM - T (no inverse), so no KSP is attached.
    ...
    def apply(
        ...
        r"""
        Apply the resolvent generator :math:`P L^* (sM - T) L P` to a
        velocity-space vector :math:`x`. Requires :meth:`update_operator` to
        have been called (done at construction); raises :class:`ValueError`
        otherwise.
    ...
    def solve(
        ...
        r"""
        Apply the resolvent :math:`L^* (sM - T)^{-1} L` to a velocity-space
        vector :math:`x`. Requires :meth:`update_operator` to have been called
        (done at construction); raises :class:`ValueError` otherwise.
