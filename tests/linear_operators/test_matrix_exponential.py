...
def test_matrix_exponential_rk2_on_vectors(comm, square_stable_random_matrix):
    r"""Test MatrixExponentialLinearOperator with RK2 on vectors"""
    tf = 2.1
    dt = 1e-5
    Apetsc, Apython = square_stable_random_matrix
    Alop = res4py.linear_operators.MatrixLinearOperator(Apetsc)
    linop = res4py.linear_operators.MatrixExponentialLinearOperator(
        Alop, tf, dt, method="RK2"
    )
    x, xpython = pytest_utils.generate_random_vector(comm, Apython.shape[-1])
...
def test_matrix_exponential_rk2_on_bvs(comm, square_stable_random_matrix):
    r"""Test MatrixExponentialLinearOperator with RK2 on BVs"""
    tf = 2.1
    dt = 1e-5
    Apetsc, Apython = square_stable_random_matrix
    Alop = res4py.linear_operators.MatrixLinearOperator(Apetsc)
...
def test_matrix_exponential_y_none(comm, square_stable_random_matrix):
    r"""Test MatrixExponentialLinearOperator with Y=None"""
    tf = 2.1
    dt = 1e-5
    Apetsc, Apython = square_stable_random_matrix
    Alop = res4py.linear_operators.MatrixLinearOperator(Apetsc)
