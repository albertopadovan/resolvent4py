...
print(
    f"Loaded SSM cache: n={n}, nf={nf}, r={r}, m={m}, "
...
def decode_phys(s, t):
    s = np.asarray(s)
    coefs = np.array(
        [np.prod(s**j) for j in multiindices],
        dtype=complex,
    )
...
print(f"    ||Im(x0_proj)||      = {np.linalg.norm(x0_proj_complex.imag):.4e}")
print(
    f"    ||P(P x) - P x||     = "
    f"{np.linalg.norm(x0_proj_twice - x0_proj_complex):.4e}"
)
# After projection, w^H x should vanish.
print(f"    |w^H x0_raw|         = {abs(np.vdot(w0_neut, x0_raw)):.4e}")
print(
    f"    |w^H x0_proj|        = {abs(np.vdot(w0_neut, x0_proj_complex)):.4e}"
)

x0_proj = x0_proj_complex.real
x0_unit = x0_proj / np.linalg.norm(x0_proj)
print(
    f"    ||x0_raw - x0_proj.real|| = {np.linalg.norm(x0_raw - x0_proj):.4e}"
)
...
print(
    f"    |s - s_rt|  (W^H)     = {np.linalg.norm(s_test - s_rt_simple):.4e}"
)
print(
    f"    |s - s_rt|  (oblique) = {np.linalg.norm(s_test - s_rt_proper):.4e}"
)
...
print(
    f"    |s0_off|             = {np.linalg.norm(s0_off):.4e} "
...
print("\n[d] Neutral content of x0_off at t=0")
print(
    f"    ||x0_off - P_neut(x0_off)|| = {np.linalg.norm(neutral_residue):.4e}"
)
...
print(
    f"    max |s_rom - s_truth(simple)| = "
    f"{np.max(np.abs(S_off - S_truth_simple)):.4e}"
)
print(
    f"    max |s_rom - s_truth(proper)| = "
...
for idx, c in zip([0, 1, 2], ["tab:blue", "tab:green", "tab:orange"]):
    ax[2].plot(
        t_eval, Vtruth[idx], color=c, lw=1.2, label=rf"$v_{{{idx + 1}}}$ truth"
    )
    ax[2].plot(
        t_eval,
        Vrom[idx],
